...
num_tests = 6;
subgrad_stepsizes = [3e-6, 4e-6, 5e-6, 6e-6, 7e-6, 8e-6];
# mirror_stepsizes = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35];
...
for i=1:num_tests
    ...
    # Run the two methods
    (subgrad_err_hist, ~) = solve_enet(Xinit, Xtrue, stoch_err,  maxIter,
                                                    fill(ηsubgrad, maxIter), method="subgradient")
    # (mirror_err_hist, ~) = solve_enet(Xinit, Xtrue, stoch_err,  maxIter,
